...
void read_lamp_values(void)  //根据传感器计算出偏移量error
{
  ...
  for(int j = 0; j < 8; j ++){     //对不同的灯判断到黑线进行相应的赋值
    if(lamp[0][j] == 1){
      ...
      if(j==0) lamp_num[0][j] = -13;//13
      else if(j==1) lamp_num[0][j] = - 8;//-8
      else if(j==2) lamp_num[0][j] = - 3;//-3
      else if(j==3) lamp_num[0][j] = - 1;//-1
      else if(j==4) lamp_num[0][j] = + 1;//-1
      else if(j==5) lamp_num[0][j] = + 3;//+3
      else if(j==6) lamp_num[0][j] = + 8;//+8
      else if(j==7) lamp_num[0][j] = + 13;//+13
	  if (positing_flag == 1) {
		  if (j == 0) lamp_num[0][j] = -10;//13
		  else if (j == 1) lamp_num[0][j] = -6;//-8
		  else if (j == 2) lamp_num[0][j] = -3;//-3
		  else if (j == 3) lamp_num[0][j] = -1;//-1
		  else if (j == 4) lamp_num[0][j] = +1;//-1
		  else if (j == 5) lamp_num[0][j] = +3;//+3
		  else if (j == 6) lamp_num[0][j] = +6;//+8
		  else if (j == 7) lamp_num[0][j] = +10;//+13
	  }
      if(j < 4)lamp_Lsum ++;
      else lamp_Rsum ++;
    }else lamp_num[0][j] = 0;
  ...
  if (lamp_Rsum > 2){
    decide = 1;
    if (lamp[0][0] == 1 && lamp[0][1] == 1 && lamp[0][2] == 1 && lamp[0][3] == 1 && lamp[0][4] == 1 && lamp[0][5] != 1) error = 0;
    else if (lamp[0][5] == 1 && lamp[0][6] != 1) error = -3;
	else if (lamp[0][3] == 1) error = 3;
    //else error = error/2;
  }else if ((lamp_Lsum < 1)&&(lamp_Rsum < 1)) {
	  if (counter_temp == 12)
	  {
		  part_flag = 4;
		  perform = 1;
 ...
 /* if (lamp[0][0] == 1 && lamp[0][1] == 1 && (part_flag ==1 || part_flag == 13 || part_flag == 3 || part_flag == 26) && once == 0) {
	  wandao_flag = 1;
  }/*/
...
void Part_Choose(void)
{
    switch(part_flag){
        ...
        case  2:
			...
			Pos_flags(120, 125);//（脉冲数，之后的速度）线上坡一段距离在加速方便上破
			Target_speed = 125;
			Posit(760, 125);//上坡距离
			Target_speed = 30;
			break;
        case 11:
			...
			NVIC_EnableIRQ(PIT2_IRQn);			          //使能PIT0_IRQn的中断	
			...
			Target_speed = 25;
			Posit(160, 20);//前进一段距离
			UART_PutStr(UART1, "#000P1503T0000!");
			Positioning();//前进到达第一个取物点
			Target_speed = 0;
			UART_PutBuff(UART3, grab_up_to_rear, 7);
			step_flag = 2;
			PIT_Delayms(PIT3, 6000);
			Target_speed = 20;
			break;
        case 12:
			Target_speed = 20;
			...
			Positioning();
			Target_speed = 0;
			UART_PutBuff(UART3, grab_up_to_front, 7);
			step_flag = 1;
			PIT_Delayms(PIT3, 6000);
			Target_speed = 20;
			break;
        case 13:
			...
			Positioning();
			Target_speed = 0;
			UART_PutBuff(UART3, grab_down_to_front, 7);
			step_flag = 3;
			PIT_Delayms(PIT3, 6000);
			Target_speed = 40;
			//Positioning_3();
			Pos_flags(1100, 65);//（脉冲数，之后的速度）
			Target_speed = 55;
			once = 0;
			positing_flag = 0;
			UART_PutBuff(UART3, smooth, 7);
			step_flag = 2;
			break;
		case  3:
			Target_speed = 70; 
        ...
        case 21:
			Target_speed = 25; 
			UART_PutStr(UART1, "#000P1507T0000!");
			Positioning_1();
...
void Pos_flags(int16 tarnum, int16 tarspeed)//（脉冲数，之后的速度）
{
	uint8 a = 1;
	Target_num = tarnum;
	pos_flag = 1;
...
void diff_speed(void){
  ...
  Left_rear_goalspeed = Target_speed * (1.2 + 0.002 * pid_track.result); Right_rear_goalspeed = Target_speed * (1.2 - 0.002 * pid_track.result);
}
...
void track_houtui() {
	int a0 = 0, a7 = 0, a1 = 0, a2 = 0, a3 = 0, a4 = 0, a5 = 0, a6 = 0, a = 0;
	a0 = GPIO_PinRead(PTB0);
	a1 = GPIO_PinRead(PTB1);
	a2 = GPIO_PinRead(PTB2);
	a3 = GPIO_PinRead(PTB3);
	a4 = GPIO_PinRead(PTB4);
	a5 = GPIO_PinRead(PTB5);
	a6 = GPIO_PinRead(PTB6);
	a7 = GPIO_PinRead(PTB7);
	if (a0 == 1 || a0 == 1 && a1 == 1 || a1 == 1 && a2 == 1 || a0 == 1 && a1 == 1 && a2 == 1 || a1 == 1 && a2 == 1 && a3 == 1) {
		a = 1;
	}
	else if (a2 == 1 && a3 == 1 ||  a2 == 1 && a3 == 1 && a4 == 1) {
		a = 2;
	}
	else if(a3 == 1 && a4 == 1) {
		a = 3;
	}
	else if (a4 == 1 && a5 == 1 || a4 == 1 && a5 == 1 && a6 == 1) {
		a = 4;
	}
	else if (a7 == 1 || a7 == 1 && a6 == 1 || a6 == 1 && a5 == 1 || a7 == 1 && a6 == 1 && a5 == 1 || a6 == 1 && a5 == 1 && a4 == 1) {
		a = 5;
	}
	else {
		a = 3;
	...
	if (a == 1) {//right
		Left_front_pwm_set(-3200);     Right_front_pwm_set(-3000);
		Left_rear_pwm_set(-3200);      Right_rear_pwm_set(-3000);
	}
	else if (a == 2) {//right
		Left_front_pwm_set(-3300);     Right_front_pwm_set(-3000);
		Left_rear_pwm_set(-3300);      Right_rear_pwm_set(-3000);
	}
	else if(a == 3) {//straight
		Left_front_pwm_set(-3300);     Right_front_pwm_set(-3000);
		Left_rear_pwm_set(-3300);      Right_rear_pwm_set(-3000);
	}
	else if (a == 4) {//left
...
void complete() {
	int a0 = 0, a1 = 0, a2 = 0, a3 = 0, a4 = 0, a5 = 0, a6 = 0, a7 = 0,i = 0;
	a0 = GPIO_PinRead(PTB0);
	a1 = GPIO_PinRead(PTB1);
	a2 = GPIO_PinRead(PTB2);
	a3 = GPIO_PinRead(PTB3);
