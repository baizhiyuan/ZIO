...
int16 Lspeed_temp_1;
int16 Rspeed_temp_1;
int16 Lspeed_temp_2 = 0;
int16 Rspeed_temp_2 = 0;
...
uint8 smooth[] =                { 0x55,0x55,0x05,0x06,0x20,0x01,0x00 };//平稳运行时

uint8 scan_up[] =               { 0x55,0x55,0x05,0x06,0x03,0x01,0x00 };//扫描上面的
uint8 scan_down[] =             { 0x55,0x55,0x05,0x06,0x04,0x01,0x00 };//扫描下面的
uint8 scan_blanking[] =         { 0x55,0x55,0x05,0x06,0x04,0x01,0x00 };//扫描下料区
...
uint8 grab_triangle_to_rear[] =       { 0x55,0x55,0x05,0x06,0x13,0x01,0x00 };//抓三角形
uint8 put_triangle_to_rear[] =       { 0x55,0x55,0x05,0x06,0x14,0x01,0x00 };//放三角形
...
int decide=0; //元素判断
int pos_flag = 0;//定位标志位
int double_flag = 0;//定位标志位
...
int temp_num = 0;//脉冲数计数
int Target_left = 0;//目标脉冲数
int Target_right = 0;//目标脉冲数
int temp_right=0;
int temp_left=0;
...
int8 rear_flag = 0;//后轮标志位
int8 wan_flag = 0;//后轮标志位
...
_pid_param_t  track_Obstacle =      //循迹PID
{
     ._kp             =  9.0 ,         //P
     ._ki             =  0.0 ,         //I
     ._kd             =  0.0 ,         //D
...
_pid_param_t  track_chasu =      //循迹PID
{
     ._kp             =  4.0 ,         //P
     ._ki             =  0.02 ,         //I
     ._kd             =  0.4 ,         //D
...
{ 
  ...
  {  
    switch(KEY_Read(0))     
    {
      case 1:LED_Reverse(1);Target_speed =50;z = 0;break;  
      case 2:UART_PutBuff(UART2, visual, 4);break; 
      case 3:step_flag = 2;break;  
      default: break; 
    }delayms(100);
  }
}
...
void Uart_Init(void)
{
    ...
    NVIC_SetPriority(UART2_RX_TX_IRQn,NVIC_EncodePriority(NVIC_GetPriorityGrouping(),1,2));     
...
void Timer_IQR_handle()
{
  ...
  if(Right_front_goalspeed >= 0){
    ...
    //Left_rear_speed = 50 * Left_rear_goalspeed - 1346;//根据实验测试电机特性曲线算得
    //Right_rear_speed = 50 * Right_rear_goalspeed - 1015;
  fenduan();
  if(Target_speed == 0){
    Left_rear_speed = 0;Right_rear_speed = 0;pid_left_front.result=0;pid_right_front.result=0;
  }//使其为0时能保证停止
  
	Left_front_pwm_set((int16)pid_left_front.result);     Right_front_pwm_set((int16)pid_right_front.result);
...
void read_lamp_values(void)  //根据传感器计算出偏移量error
{
 ...
 /* if(dingwei_flag == 0){
  ...
  if(lamp[0][5] != 1 && lamp[0][6] != 1 && lamp[0][7] != 1 && lamp[0][4] != 1){
    if(part_flag == 26){
       part_flag = 4;
       perform = 1;
      /*PIT_Delayms(PIT3,700);
      Target_speed = 0;
...
void target_slow(int16 tar,int time){
  int a = 0;
  while(a){
...
void target_slow_t(int16 tar,int time){
  int a = 0;
  while(a){
...
void Part_Choose(void)
{
  switch(part_flag){
    ...
    case  2:
      ...
      //dingwei_flag = 1;
			//Posit(90, 70);//（脉冲数，之后的速度）线上坡一段距离在加速方便上破
     // dingwei_flag = 0;
      //target_slow(110,3);
			Posit_up(700, 110);//上坡距离
      Target_speed = 35;
      wan_flag = 0;
      //dingwei_flag = 1;
      //target_slow(30,200);
      ...
      dingwei_flag = 1;
			...
			break;
    case 11:
      Posit(35, 20);target_slow(0,10);Target_speed = 0;UART_PutStr(UART1, "#000P1503T0000!");
			material_bi_feeding();
			Target_speed = 20;
      positing_flag = 1;
			break;
    ...
    case 13:
      ...
      step_flag = 2;UART_PutBuff(UART3, smooth, 7);           //摆放机械臂调整物料槽，平稳运行
			Posit_up(1000, 30);                                     //匀速缓慢下坡
      Target_speed = 50;
      wan_flag = 1;positing_flag = 0;dingwei_flag = 0;        //标志位改变
			break;
		case  3:
			UART_PutStr(UART1, "#000P1502T0000!"); 
		  Obstacle();       //循迹测距    
      rotating();       //避障
			Posit(1500, 50);        //一段距离的速度
      wan_flag = 0;           //改用定位的PID
      positing_flag = 1;dingwei_flag = 0;Target_speed = 40;
			break;
      //Target_speed = 60;Obstacle();rotating();Target_speed = 50;break;
      //Obstacle();break;//避障区域
      // Target_speed = 100;track_PID ();counter_sum();break; 
    case 21:
			Target_speed = 25; xialiao_flag = 0;//速度和标志位改变
      Posit(40, 20);target_slow(0,5);Target_speed = 0;UART_PutStr(UART1, "#000P1507T0000!");
...
void Posit_up(int16 tarnum,int16 tarspeed)
{
	uint8 a = 1;
	Target_speed = tarspeed;
	Target_num = tarnum;
...
void turn_left(void){
  ...
  //wan_flag = 2;
  //diff_speed();
  Left_front_goalspeed = Target_speed*0.1; Right_front_goalspeed = Target_speed*3;
  Left_rear_goalspeed = Target_speed*0.1; Right_rear_goalspeed = Target_speed*3;
}
void turn_right(void){
  ...
  //wan_flag = 2;
  //diff_speed();
  Left_front_goalspeed = Target_speed*3; Right_front_goalspeed = -Target_speed*0.1;
  Left_rear_goalspeed = Target_speed*3; Right_rear_goalspeed = -Target_speed*0.1;
}
void run_straight(void){
  ...
  //wan_flag = 2;
  //diff_speed();
  Left_front_goalspeed = Target_speed; Right_front_goalspeed = Target_speed;
  Left_rear_goalspeed = Target_speed; Right_rear_goalspeed = Target_speed;
}
...
void rotating(void){
  ...
  while (a){
    if(c==1){   //左转
      ...
      lamp[0][8] = GPIO_PinRead(PTC13);
	  lamp[0][7] = GPIO_PinRead(PTC14);
      if(lamp[0][7] == 1 && lamp[0][8] == 1)
      {
      //if(double_flag != 1)
     // {
        Target_left=350;
...
void diff_speed(void){
  ...
  float pid_error;
  if(pit2_flag==1){
    if(wan_flag == 0) pid_error = get_pid(&track_bzy,error); 
    else if(wan_flag == 1) pid_error = get_pid(&track_wan,error);
    else if(wan_flag == 2) pid_error = get_pid(&track_Obstacle,error);
...
uint8 grab_up_to_rear[] =       { 0x55,0x55,0x05,0x06,0x06,0x01,0x00 };//抓上面的到里面的盒子
uint8 grab_down_to_front[] =    { 0x55,0x55,0x05,0x06,0x07,0x01,0x00 };//抓下面的到外面的盒子
uint8 grab_down_to_rear[] =  
-------------------------------------------------------------------------------------------------------*/
int supplies_bi[3][4]= {
  {1, 1, 2, 2},             //第一个数：物料所在位置；第二个数：物料下料区位置，第三个数：物料形状，第四个数：物料盒位置
  {4, 3, 5, 3},
  {5, 6, 4, 1},
};
...
void material_bi_feeding(void){                        //上料区扫描识别抓取
  ...
  if(supplies_temp != 7)   {//这次有收获
    UART_PutStr(UART1, "#000P1503T0000!");          //语音播报成功
    if(supplies_bi[down][3] == 2)   UART_PutBuff(UART3, grab_down_to_rear, 7);    //如果是中间盒子需要放里面,抓下面的到里面的盒子
    else UART_PutBuff(UART3, grab_down_to_front, 7);                //抓下面的盒子到外面
    PIT_Delayms(PIT3,4000);
  } else UART_PutStr(UART1, "#000P1503T0000!");          //语音播报失败
  ...
  if(supplies_temp != 7 && supplies_temp != down)   {//这次有收获
    UART_PutStr(UART1, "#000P1503T0000!");          //语音播报成功
    if(supplies_bi[up][3] == 2)   UART_PutBuff(UART3, grab_up_to_rear, 7);    //如果是中间盒子需要放里面,抓上面的到里面的盒子
...
void material_bi_blanking(void){                        //下料区扫描识别放置
  ...
  switch (part_flag)
  {
  case 21:blanking = 1;break;
  case 22:blanking = 2;break;
  case 23:blanking = 3;break;
  case 24:blanking = 4;break;
  case 25:blanking = 5;break;
  case 26:blanking = 6;break;
  default:blanking = 7;break;
  }

  UART_PutStr(UART1, "#000P1503T0000!");          //语音播报识别
  UART_PutBuff(UART3, scan_blanking, 7);        //识别下面
  PIT_Delayms(PIT3, 500);                         //等待它动作稳定后拍照
  ...
  if(supplies_temp != 7)   {//这次有收获
    UART_PutStr(UART1, "#000P1503T0000!");          //语音播报成功
    if(supplies_bi[blanking][3] == 2)   UART_PutBuff(UART3, put_rear, 7);    //放里面的物料
    else UART_PutBuff(UART3, put_front, 7);                //放外面的物料
    PIT_Delayms(PIT3,4000);
  } else UART_PutStr(UART1, "#000P1503T0000!");          //语音播报失败
