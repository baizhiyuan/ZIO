...
int const supplies_2[3][4]= {
  {2, 1, 0, 3},             //第一个数：物料所在位置；第二个数：物料下料区位置，第三个数：物料形状，第四个数：物料盒位置
  {4, 2, 1, 1},
  {1, 4, 3 ,2},
};int const supplies_1[3][4]= {
  {2, 1, 0, 3},             //第一个数：物料所在位置；第二个数：物料下料区位置，第三个数：物料形状，第四个数：物料盒位置
  {4, 2, 1, 1},
  {1, 4, 3 ,2},
};
...
volatile uint8_t double_flag = 0;//定位标志位
...
{ 
  ...
  {  
    switch(KEY_Read(0))     
    {
      ...
      case 3:UART_PutBuff(UART3, grab_down_to_rear, 7);break;
      default: break; 
    }delayms(100);
    if(GPIO_PinRead(PTD0)==1){
      delayms(1000);
